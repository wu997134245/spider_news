...
class Connection(object):
    ...
    def __init__(self, host, database, user=None, password=None,
        ...
        args = dict( maxconnections = 5,  conv=CONVERSIONS, use_unicode=True, charset=charset,
    ...
    def init_pool(self):
        self.close()
        self._db = PooledDB(pymysql, **self._db_args)
    ...
    def query(self, query, *parameters, **kwparameters):
        ...
        try:
            ...
            #return [OrderedDict(zip(column_names, row)) for row in cursor]
            return [OrderedDict(itertools.izip(column_names, row)) for row in cursor]
        except:
            cursor.close()
            conn.close()


    def query_re_column(self, query, *parameters, **kwparameters):
        """Returns a row list for the given query and parameters."""
        conn, cursor = self._cursor()
        try:
            self._execute(cursor, query, parameters, kwparameters)
            column_names = [d[0] for d in cursor.description]
            return {'data':[OrderedDict(itertools.izip(column_names, row)) for row in cursor],'columns':column_names}
        except:
            cursor.close()
            conn.close()
